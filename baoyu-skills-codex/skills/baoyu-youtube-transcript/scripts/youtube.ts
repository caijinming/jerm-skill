...
let cachedYtDlpCommand:
  | { command: string; args: string[]; label: string; supportsAdvancedFlags: boolean }
  | null
  | undefined;
...
export async function fetchTranscriptSnippets(
  ...
  if (url.startsWith("file://")) {
    ...
    return {
      snippets: parseTranscriptPayload(await Bun.file(filePath).text(), filePath),
      language,
      languageCode,
    };
  }
...
function attachDownloadedSubtitleFiles(
  info: YtDlpInfo,
  videoId: string,
  command: { command: string; args: string[]; label: string; supportsAdvancedFlags: boolean }
): YtDlpInfo {
  const dir = mkdtempSync(join(tmpdir(), "yt-dlp-subs-"));
  const args = [
    ...command.args,
    "--skip-download",
    "--write-subs",
  ...
  if (command.supportsAdvancedFlags) {
    args.push("--js-runtimes", "bun", "--remote-components", "ejs:github");
  }
  ...
  for (const file of readdirSync(dir)) {
    ...
    automatic[languageCode] = automatic[languageCode] || [];
    automatic[languageCode].push({
      ext,
  ...
  return { ...info, automatic_captions: automatic };
}
...
export function detectYtDlpCommand():
  | { command: string; args: string[]; label: string; supportsAdvancedFlags: boolean }
  | null {
  ...
  for (const candidate of candidates) {
    ...
    cachedYtDlpCommand = { ...candidate, supportsAdvancedFlags: supportsRequiredFlags };
...
function fetchYtDlpInfo(videoId: string): YtDlpInfo {
  ...
  return hasTracks ? info : attachDownloadedSubtitleFiles(info, videoId, command);
}

...
function findFirstImageUrl(document: any): string | null {
...
async function convertTweetViaBrowserFallback(
  inputUrl: string,
  log: (message: string) => void
): Promise<string | null> {
  const cliPath = path.resolve(
    path.dirname(process.argv[1] ?? path.resolve(process.cwd(), "scripts/main.ts")),
    "../../baoyu-url-to-markdown/scripts/vendor/baoyu-fetch/src/cli.ts"
  );

  if (!fs.existsSync(cliPath)) {
    log(`[x-to-markdown] Browser fallback unavailable; missing CLI: ${cliPath}`);
    return null;
  }

  const result = spawnSync(
    process.execPath,
    [cliPath, inputUrl, "--adapter", "x", "--wait-for", "interaction", "--json"],
    {
      encoding: "utf8",
      env: { ...process.env },
    }
  ...
  if (result.status !== 0) {
    ...
    if (stderr) {
      log(`[x-to-markdown] Browser fallback failed: ${stderr}`);
    }
    return null;
  }
  ...
  try {
    ...
    log(
      `[x-to-markdown] Browser fallback returned invalid JSON: ${error instanceof Error ? error.message : String(error)}`
    );
    return null;
  }

  const body = typeof parsed?.markdown === "string" ? parsed.markdown.trimEnd() : "";
  ...
  const meta = formatMetaMarkdown({
    url: typeof document?.canonicalUrl === "string" ? document.canonicalUrl : inputUrl,
    requestedUrl: inputUrl,
    title: typeof document?.title === "string" ? document.title : null,
    author: typeof document?.author === "string" ? document.author : null,
    coverImage: findFirstImageUrl(document),
  });

  log("[x-to-markdown] Browser fallback succeeded via baoyu-fetch.");
...
async function main(): Promise<void> {
  ...
  if (kind === "article" && articleId) {
    markdown = await convertArticleToMarkdown(normalizedUrl, articleId, log);
  } else {
    try {
      markdown = await tweetToMarkdown(normalizedUrl, { log });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error ?? "");
      log(`[x-to-markdown] Primary tweet fetch failed: ${message}`);
      const fallbackMarkdown = await convertTweetViaBrowserFallback(normalizedUrl, log);
      if (!fallbackMarkdown) {
        throw error;
      }
